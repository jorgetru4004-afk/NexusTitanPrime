...
async function runAI3_Risk(sym, dir, snap, sizeData) {
  const portfolioClean = portfolioHeat < 0.05 && dailyLoss < 50;
  const prompt = `You are TITAN PRIME AI #3 — RISK MANAGER for large cap trades.
...
Stop: $${dir==='SHORT'?(snap.price*1.04).toFixed(2):(snap.price*0.95).toFixed(2)} Target: $${dir==='SHORT'?(snap.price*0.92).toFixed(2):(snap.price*1.08).toFixed(2)}
VIX: ${vixLevel} | Macro risk: ${macroContext.keyRisk||'unknown'}
Portfolio status: ${portfolioClean ? 'CLEAN — zero heat, zero daily loss. This is a fresh start.' : 'ACTIVE — review carefully.'}
IMPORTANT: If portfolio heat is below 10% AND daily loss is below $50, the risk profile is LOW by default.
Only reject if: heat > 40%, daily loss > 60% of limit, or stop placement is genuinely invalid.
Do NOT reject a clean portfolio trade out of general caution.
Return ONLY JSON: {"verdict":"YES"|"NO","confidence":65-90,"reason":"risk assessment","riskRating":"LOW|MEDIUM|HIGH","holdPeriod":"INTRADAY|SWING|POSITION"}`;
  try {
    const result = await callClaude(prompt, 220);
    const dec = parseJSON(result);
    if (!dec) return null;
    ...
    console.log(`🧠 AI3 ${sym}: ${dec.verdict} (${dec.confidence}%) [${dec.riskRating}] ${dec.holdPeriod}`);
    return dec;
  } catch (e) { return null; }
}
...
async function runPrimeScanner() {
  ...
  for (const [sym, snap] of available) {
    ...
    // Signal 1: Institutional accumulation — high volume + positive move
    if (snap.change >= SETTINGS.minMoveThreshold && snap.volMultiple >= SETTINGS.minVolMultiple * 1.5) {
    ...
    // Signal 2: Sector ETF momentum — SPY/QQQ/sector ETFs
    if (['SPY', 'QQQ', 'IWM', 'XLF', 'XLE', 'GLD'].includes(sym) && snap.change >= 0.5 && snap.volMultiple >= 1.2) {
      scanCandidates.push({ sym, snap, dir: 'LONG', patternType: 'ETF_MOMENTUM', score: 80, reason: `ETF momentum: ${sym} +${snap.change}%` });
    }

    // Signal 3: Mag7 breakout
    if (['NVDA', 'MSFT', 'AAPL', 'GOOGL', 'META', 'AMZN', 'TSLA'].includes(sym) && snap.change >= 2 && snap.volMultiple >= 1.3) {
    ...
    // Signal 4: Technical breakout — new high on volume
    if (snap.price >= snap.high * 0.998 && snap.change >= 1.5 && snap.volMultiple >= 2) {
    ...
    // Signal 5: Distribution short — large drop on high volume
    if (snap.change <= -3 && snap.volMultiple >= 2 && marketRegime !== 'BULL_TRENDING') {
  ...
  console.log(`📊 PRIME scan: ${scanCandidates.length} candidates | SPY:${spyChange}% | Regime:${marketRegime}`);
